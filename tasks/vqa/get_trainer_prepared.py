...
from .dataset_brote import BroteDataset
from model.utils_prepared import get_model
from training.trainer_blip2 import BLIP2Trainer
from training.trainer_instructblip_ex import InstructBLIP2Trainer
from model.instructblip_ex.processing_instructblip import InstructBlipProcessor
from model.instructblip_ex.configuration_instructblip import InstructBlipConfig
...
def get_trainer(args):
    ...
    if model_args.global_calculation is not None:
        config.qformer_config.global_calculation = model_args.global_calculation
    else:
        config.qformer_config.global_calculation = 'concat'
    # the following configs are deprecated
    config.qformer_config.send_condition_to_llm = model_args.send_condition_to_llm
    config.qformer_config.both_condition = True if model_args.condition_from == 'both' else False

    model = get_model(model_args, training_args.local_rank, config)
    ...
    model.set_image_placeholder("图")
    model.set_loss_type(training_args.dual_loss)
    
    dataset = BroteDataset(processor, model_args, data_args, training_args, config)
    ...
    return model, trainer, dataset.predict_dataset
