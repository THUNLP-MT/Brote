...
def convert_blip2_checkpoint(model_name, pytorch_dump_folder_path=None, push_to_hub=False):
    ...
    hf_model_device = "cuda:5" if torch.cuda.is_available() else "cpu"
    lavis_device = "cuda:6" if torch.cuda.is_available() else "cpu"
    #original_model, vis_processors, _ = load_model_and_preprocess(
    #    name=name, model_type=type, is_eval=True, device=lavis_device
    #)
    #original_model.eval()
    bin_files_path = '/data/users/air/wangsiyu/wzy/MMICL/models/models--Salesforce--instructblip-flan-t5-xl/snapshots/6c0cf6bef6330a114473cb5cec43d7beeb2a74ac'
    state_dict = {}
    for bin_file in sorted(os.listdir(bin_files_path)):
        if bin_file.endswith(".bin"):
            model_state_dict = torch.load(os.path.join(bin_files_path, bin_file), map_location=lavis_device)
...
if __name__ == "__main__":
    ...
    convert_blip2_checkpoint(args.model_name, args.pytorch_dump_folder_path)
