...
class BaseModelOutputWithPoolingAndCrossAttentions(ModelOutput):
    ...
    global_conditions_to_llm: Optional[torch.FloatTensor] = None
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, InstructBlipVisionEmbeddings):
            ...
            if is_deepspeed_zero3_enabled():
                nn.init.normal_(module.position_embedding, mean=0.0, std=factor)
                nn.init.normal_(module.class_embedding, mean=0.0, std=factor)
            else:
...
class InstructBlipQFormerEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #self.pre_cond_layernorm = nn.LayerNorm(config.proj_in_dim, eps=config.layer_norm_eps)
        if self.config.both_condition:
            self.condition_projection = nn.Linear(config.proj_in_dim, config.hidden_size)
            self.condition_mid_projection = nn.Linear(config.proj_in_dim, config.hidden_size)
        else:
            self.condition_projection = nn.Linear(config.proj_in_dim, config.hidden_size)
    ...
    def forward(
        ...
        position_ids=None,
        query_embeds=None,
        global_conditions = None, # add condition context to the input
        global_conditions_mid = None, # add condition context generated from the middle layer to the input
        image_counts_per_instance = None, # count the number of image in each input (for expanding the global_condition dimension)
        ...
        if input_ids is not None:
            ...
            embeddings = query_embeds # [n_images, 32, 768]

        if global_conditions is not None:
            if self.config.both_condition:
                global_conditions = self.condition_projection(global_conditions)
                global_conditions_mid = self.condition_mid_projection(global_conditions_mid)
            else:
                global_conditions = self.condition_projection(global_conditions)

            if image_counts_per_instance is None:
                if self.config.both_condition:
                    global_conditions = global_conditions.expand(embeddings.shape[0], -1, -1)
                    global_conditions_mid = global_conditions_mid.expand(embeddings.shape[0], -1, -1)
                else:
                    global_conditions = global_conditions.expand(embeddings.shape[0], -1, -1)

            else:
                padding_instance_count = embeddings.shape[0] - sum(image_counts_per_instance)
                image_counts_per_instance[-1] = image_counts_per_instance[-1]+padding_instance_count

                if self.config.both_condition:
                    global_conditions = [_cond.expand(image_counts_per_instance[i], -1, -1) for i, _cond in enumerate(global_conditions)]
                    global_conditions = torch.concat(global_conditions, 0)
                    global_conditions_mid = [_cond.expand(image_counts_per_instance[i], -1, -1) for i, _cond in enumerate(global_conditions_mid)]
                    global_conditions_mid = torch.concat(global_conditions_mid, 0)
                else:
                    global_conditions = [_cond.expand(image_counts_per_instance[i], -1, -1) for i, _cond in enumerate(global_conditions)]
                    global_conditions = torch.concat(global_conditions, 0)

            if self.config.send_condition_to_llm:
                global_conditions_to_llm = global_conditions.detach()

            if self.config.global_calculation == 'concat':
                embeddings = torch.cat((embeddings, global_conditions), dim=1) # [n_images, 33, 768] 
            elif self.config.global_calculation == 'add':
                if self.config.both_condition:
                    global_conditions = global_conditions.expand(-1, embeddings.shape[1], -1) 
                    global_conditions_mid = global_conditions_mid.expand(-1, embeddings.shape[1], -1) 
                    embeddings = embeddings + global_conditions + global_conditions_mid
                else:
                    global_conditions = global_conditions.expand(-1, embeddings.shape[1], -1) 
                    embeddings = embeddings + global_conditions

        embeddings = self.layernorm(embeddings)
        embeddings = self.dropout(embeddings)

        if self.config.send_condition_to_llm:
            return embeddings, global_conditions_to_llm
...
class InstructBlipQFormerModel(InstructBlipPreTrainedModel):
    ...
    def forward(
        ...
        global_conditions = None, # add condition context to the input
        global_conditions_mid = None, # add condition context generated from the middle layer to the input
        image_counts_per_instance = None, # count the number of image in each input (for expanding the global_condition dim0)
        head_mask=None,
        encoder_hidden_states=None,
        encoder_attention_mask=None,
        past_key_values=None,
        ...
        if global_conditions is not None:
            # for the 2nd forward round
            if self.config.send_condition_to_llm:
                embedding_output, global_conditions_to_llm = self.embeddings(
                    input_ids=input_ids,
                    position_ids=position_ids,
                    query_embeds=query_embeds,
                    global_conditions = global_conditions,
                    global_conditions_mid = global_conditions_mid,
                    image_counts_per_instance = image_counts_per_instance,
        ...
        if not self.config.send_condition_to_llm:
            global_conditions_to_llm = None

        return BaseModelOutputWithPoolingAndCrossAttentions(
            global_conditions_to_llm=global_conditions_to_llm,
...
class InstructBlipForConditionalGeneration(InstructBlipPreTrainedModel):
    ...
    def __init__(self, config: InstructBlipConfig):
        ...
        self.text_eos_token_id = config.text_config.eos_token_id
        self.global_calculation = config.qformer_config.global_calculation
        self.send_condition_to_llm = config.qformer_config.send_condition_to_llm 
        self.both_condition = True if config.qformer_config.both_condition else False
    ...
    def set_image_placeholder(self, placeholder):
        self.image_placeholder = placeholder

    def set_processor(self, processor):
        self.processor = processor

    def set_loss_type(self, dual_loss_flag):
        if dual_loss_flag:
            self.loss_type = "dual"
        else:
    ...
    def tokenize2nd(self, condition_len, input_text, device, padding=True, is_training=False, max_length=None):
        # add global condition vectors to each of the query vectors
        # re-tokenize (considerin the condition len) and extend mask shape
        if self.image_placeholder in input_text[0]:
            replace_placeholder = self.image_placeholder
        else:
            replace_placeholder = '{replace_token}'

        replace_token = "".join((32+condition_len)*[self.image_placeholder])
        input_text = [tt.replace(replace_placeholder, replace_token) for tt in input_text]
        inputs = self.processor(text=input_text, return_tensors="pt", padding=padding, max_length=max_length, truncation=True).to(device)
    ...
    def forward(
        ...
        image_placeholder: str = None,
        pixel_values: torch.FloatTensor= None,
        image_counts_per_instance: Optional[list] = None,
        global_conditions: Optional[torch.LongTensor] = None,
        global_conditions_mid: Optional[torch.LongTensor] = None,
        ...
        # step 0: get the global outputs for the 1st forward without gradients
        if global_conditions is not None:
            if self.both_condition:
                assert global_conditions is not None and global_conditions_mid is not None
                global_conditions = global_conditions.unsqueeze(1)
                global_conditions_mid = global_conditions_mid.unsqueeze(1)
                condition_len = global_conditions.shape[1]
            else:
                global_conditions = global_conditions.unsqueeze(1)
                condition_len = global_conditions.shape[1]
        ...
        if pixel_values is None:
            ...
            if qformer_input_ids is not None:
                ...
                query_outputs = self.qformer(
                    ...
                    global_conditions=global_conditions, # add global condition for the 2nd forward
                    global_conditions_mid=global_conditions_mid, # add global condition for the 2nd forward
                    image_counts_per_instance = img_count,
                ...
                if return_dict:
                    ...
                    query_output = query_outputs[1]

            # step 3: use the language model, conditioned on the query outputs and the prompt
            if self.send_condition_to_llm:
                global_conditions_to_llm = query_outputs.global_conditions_to_llm
                query_output = torch.cat((global_conditions_to_llm, query_output), dim=1) # [n_images, 33, 768] 
            language_model_inputs = self.language_projection(query_output)

            if global_conditions is not None: 
                # expand tokens for global_conditions "concat" calculatino
                if self.global_calculation == "concat":
                    input_ids, attention_mask = self.tokenize2nd(condition_len, input_text, language_model_inputs.device, padding="max_length", is_training=True, max_length=input_ids.shape[1])
            ...
            # if encounter errors on dimensions
            # if not (language_model_inputs.shape[0]*language_model_inputs.shape[1]) == image_embeds_index[0].shape[0]:
            #     print(batch_size, language_model_inputs.shape, image_embeds_index[0].shape)
            #     print(input_text)
        ...
        **generate_kwargs,
        ...
        if global_conditions is not None:
            ...
            if self.both_condition:
                global_conditions_mid = global_conditions_mid.unsqueeze(1)
            condition_len = global_conditions.shape[1]
            # get image_counts_per_instance:
            if image_counts_per_instance is None:
                if img_no_start_from == 1:
                    image_counts_per_instance = input_ids.max(axis=1).values - (sp_token+1)
                else:
                    image_counts_per_instance = input_ids.max(axis=1).values - sp_token

        caption_flag = False
        batch_size = input_ids.shape[0] 
        if input_ids is None: # [batch_size, seq_len]
            input_ids = (
            ...
            caption_flag = True
        if pixel_values is None: # [1, n_images_in_batch, 3, 224, 224]
            inputs_embeds = self.language_model.get_input_embeddings()(input_ids)    
            if attention_mask is None:
                attention_mask = torch.ones_like(input_ids)
            attention_mask = attention_mask.to(inputs_embeds.device)
            ...
            if img_mask is None:
                img_mask = torch.ones(pixel_values.shape[:2])
            img_count = img_mask.sum(1)
            pixel_values = pixel_values[img_mask.bool()] # [n_images, 3, 224, 224]
            
            image_embeds = self.vision_model(pixel_values, return_dict=True).last_hidden_state # [n_images, 257, 1408]

            image_attention_mask = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image_embeds.device) # [n_images, 257]

            query_tokens = self.query_tokens.expand(image_embeds.shape[0], -1, -1) # [n_images, 32, 768]
                
            query_attention_mask = torch.ones(query_tokens.size()[:-1], dtype=torch.long, device=image_embeds.device) # [n_images, 32]
            if qformer_input_ids is not None:
                ...
                query_output = query_outputs.last_hidden_state  # [n_images, 32, 768]          

            if self.send_condition_to_llm:
                global_conditions_to_llm = query_outputs.global_conditions_to_llm
                query_output = torch.cat((global_conditions_to_llm, query_output), dim=1) # [n_images, 33, 768] 
            language_model_inputs = self.language_projection(query_output) # [n_images, 32, 2048] # 33 for to_llm

            if global_conditions is not None: 
                if self.global_calculation == "concat": # reshape for "concat"
                    input_ids, attention_mask = self.tokenize2nd(condition_len, input_text, language_model_inputs.device)

            if attention_mask is None: # [batch_size, input_tokens(text+image)]
                attention_mask = torch.ones_like(input_ids)
                attention_mask = attention_mask.to(language_model_inputs.device)
