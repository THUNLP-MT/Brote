...
class InstructBlipForConditionalGenerationModelOutput(ModelOutput):
    ...
    condition_loss: Optional[Tuple[torch.FloatTensor]] = None
    task_loss: Optional[Tuple[torch.FloatTensor]] = None
...
class InstructBlipQFormerEmbeddings(nn.Module):
    ...
    def forward(
        ...
        position_ids=None,
        query_embeds=None,
        global_conditions = None, # add global_conditions input
        ...
        if global_conditions is not None:
            ...
            if self.config.global_calculation == 'concat':
                embeddings = torch.cat((embeddings, global_conditions), dim=1) 
...
class InstructBlipQFormerModel(InstructBlipPreTrainedModel):
    ...
    def __init__(self, config: InstructBlipQFormerConfig):
        ...
        #self.projection = nn.Linear(config.projection_input, config.projection_output)
    ...
    def forward(
        ...
        if global_conditions is not None or skip_condition:
            # TODO: for the concentrating phase
...
class InstructBlipForConditionalGeneration(InstructBlipPreTrainedModel):
    ...
    def __init__(self, config: InstructBlipConfig):
        ...
        self.mode = 'explicit' # default mode is 'explicit'
    ...
    def set_loss_type(self, dual_loss_flag):
        # set loss type, "dual" for implicit mode 
        if dual_loss_flag:
            self.loss_type = "dual"
        else:
    ...
    def forward(
        ...
        Args:
            global_conditions (`torch.FloatTensor` of shape (batch_size, dim_feats)):
                Input condition_context_vector
            global_conditions_mid (`torch.FloatTensor` of shape (batch_size, dim_feats)):
                Input condition_context_vector from different layers
            pixel_values (`torch.FloatTensor` of shape (batch_size, num_channels, height, width)):
        ...
        # step 0: get the global outputs for the browsing phase without gradients
        if self.mode == 'implicit':
            with torch.no_grad():
                condition_loss, global_conditions = self.forward_wo_grad(
                    pixel_values = pixel_values,
                    ...
                    return_dict=True,
                    img_mask = img_mask,
                    set_min_padding_size = set_min_padding_size,
                    sp_token = sp_token
                    ) 
                #torch.cuda.empty_cache()
                eos_token_index = torch.where(input_ids == self.text_eos_token_id) 
                global_conditions = global_conditions.encoder_last_hidden_state[eos_token_index].unsqueeze(1)
                condition_len = global_conditions.shape[1]
        else: # self.mode == 'explicit'
            assert global_conditions is not None
        ...
        if pixel_values is None:
            ...
            # step 2: forward the query tokens through the QFormer and integrate with condition_contex_vector, then using the image embeddings for cross-attention
            ...
            if qformer_input_ids is not None:
                ...
                query_outputs = self.qformer(
                    ...
                    attention_mask=qformer_mask,
                    query_embeds=query_tokens,
                    global_conditions=global_conditions, # add global condition for the concentrating phase
        ...
        if self.config.use_decoder_only_language_model:
            ...
            if labels is not None:
                ...
                # Flatten the tokens
                loss_fct = CrossEntropyLoss(reduction="mean")
                task_loss = loss_fct(shift_logits.view(-1, self.config.text_config.vocab_size), shift_labels.view(-1))
        else:
            outputs = self.language_model(
                inputs_embeds=inputs_embeds,
                attention_mask=attention_mask,
            ...
            task_loss = outputs.loss if return_dict else outputs[0]
            logits = outputs.logits if return_dict else outputs[1]

        if self.loss_type == 'dual':
            # dual loss for implicit mode
            loss = task_loss + condition_loss
        else:
            loss = task_loss
        ...
        if not return_dict:
            ...
            if self.loss_type == 'dual':
                # dual loss for implicit mode
                return ((loss, task_loss, condition_loss,) + output) if loss is not None else output
            else:
                return ((task_loss,) + output) if task_loss is not None else output
        ...
        **generate_kwargs,
        ...
        Args:
            ...
            attention_mask (`torch.LongTensor` of shape (batch_size, sequence_length), *optional*):
                Mask to avoid performing attention on padding token indices.
            return_logits:
                Whether to return final logits. 
        ...
        if not skip_condition and self.mode == 'implicit':
            if global_conditions is None and first_round==False:
                global_conditions = self.generate(
                        pixel_values = pixel_values,
                        first_round=True,
                        qformer_input_ids = qformer_input_ids,
                        qformer_attention_mask = qformer_attention_mask,
            ...
            assert global_conditions is None
        ...
        if first_round==True and global_conditions is None:
            # calculate the browsing resutls
            outputs = self.language_model(
                inputs_embeds=inputs_embeds,
            ...
            if return_logits:
                # return logits in case you want to obtain the probability for certain tokens
                outputs = self.language_model(
                    inputs_embeds=inputs_embeds,
                    decoder_inputs_embeds = inputs_embeds,
                    attention_mask=attention_mask,
                ...
                # calculate the prediction of concentrating phase
                # obtain the predicted token ids
